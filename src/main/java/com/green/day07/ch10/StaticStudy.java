...
public class StaticStudy {
    ...
    public static void main(String[] args) {
        NonStaticNumBox nnb1 = new NonStaticNumBox(); //각각 다른 공간이라 다른 값 저장
        nnb1.n1 = 10;
        nnb1.n2 = 30;

        NonStaticNumBox nnb2 = new NonStaticNumBox(); //각각 다른 공간이라 다른 값 저장
        nnb2.n1 = 15;
        nnb2.n2 = 20;
        ...
        snb1.sum();
        snb2.sum();  //static은 같은 공간을 공유하기 때문에 마지막 수정된 값 적용
        /*
        객체마다 멤버필드의 공간이 각각 생기길 원하면 non-static(인스턴스 멤버필드)
        로 해야한다. (각 객체마다 다른 값을 저장 가능하게 하고 싶다면)
...
class StaticNumBox {
    ...
    static void sum() {
       ...
       // hello(); //static 메소드에서 인스턴스 메소드 호출 불가
        // "parameter"만 다르면 overloading 가능 (호출할 때 구분이 가능하면, 다른 조건은 상관 x)
        }
...
class NonStaticNumBox {
    ...
    void sum() {
        System.out.println(n1 + n2);
        System.out.println(n3 + n4); //인스턴스 메소드에서 클래식 멤버필드 접근 가능하다.
        hello(); //인스턴스 메소드에서 클래스 멤버메소드 호출 가능하다.
        // 왜 가능하냐 sum()메소드가 호출이 된다는 것은 NonStaticNumBox가 객체화가 되었다는 것이고 static은 이미 메모리에 올라가 있기 때문에 사용이 가능하다.
    }
